...
def test_make_plot():
    # create some test data
    det = 'H1'
    strain_H1_whitenbp = np.random.randn(1000)
    strain_L1_whitenbp = np.random.randn(1000)
    template_match = np.random.randn(1000)
    time = np.linspace(-1, 1, 1000)
    timemax = 0.5
    SNR = np.abs(np.random.randn(1000))
    eventname = 'test_event'
    plottype = 'png'
    tevent = 0.2
    template_fft = np.fft.rfft(template_match)
    datafreq = np.fft.rfftfreq(len(strain_H1_whitenbp), d=1.0/1000)
    d_eff = 1.0
    freqs = datafreq+1
